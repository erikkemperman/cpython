...
#include "pycore_unionobject.h"    // _Py_union_type_or, _PyUnion_Check
...
{
    int is_rich = (_PyGenericAlias_Check(left) && _PyGenericAlias_Check(right))
                    || (_PyUnion_Check(left) && _PyUnion_Check(right));
    return is_rich ? PyObject_RichCompareBool(left, right, Py_EQ) : left == right;
}
...
exit:
    ...
    rc = (parens && _PyUnicodeWriter_WriteASCIIString(writer, "(", 1))
        || _PyUnicodeWriter_WriteStr(writer, r)
        || (parens && _PyUnicodeWriter_WriteASCIIString(writer, ")", 1));
    Py_DECREF(r);
    return rc;
}
...
PyTypeObject _PyIntersection_Type = {
    ...
    .tp_doc = PyDoc_STR("Represent a PEP XYZ Intersection type\n"
              "\n"
              "E.g. for int & str"),
